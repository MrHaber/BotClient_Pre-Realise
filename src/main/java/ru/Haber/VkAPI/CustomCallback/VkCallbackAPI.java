...
import ru.Haber.VkAPI.bootstrap.BotClient.BotUtilsImpl;
import ru.Haber.VkAPI.bootstrap.BotClient.IBotUtils;
import ru.Haber.VkAPI.bootstrap.BotClient.VkCallbackListener;
...
public class VkCallbackAPI {
    ...
    @Getter
    public final static List<VkCallbackListener> lists = Lists.newArrayList();
    static {
        Map<String, Type> types = new HashMap<>();
        types.put(CALLBACK_EVENT_MESSAGE_NEW, new TypeToken<CallbackMessage<Message>>() {
        }.getType());
        types.put(CALLBACK_EVENT_MESSAGE_EVENT, new TypeToken<VkCustomCallbackMessage<VkCallbackMessage>>() {
        }.getType());
    ...
    public void messageEvent(Integer groupId, VkCallbackMessage message,IBotUtils utils) {
    	
    	lists.parallelStream().forEach(elements -> {
    		
    		val classes = elements.getClass();
    		
    		val methods = Arrays.asList(classes.getMethods());
    		
    		methods.parallelStream().forEach(method-> {
    			
    			method.setAccessible(true);
    			if(method.getName().equalsIgnoreCase("onCallbackEvent")) {
    				try {
						method.invoke(elements, groupId,message,utils);
						
					} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
						
						BotClient.getLogger().info("[VKAPI] Cause somethink wrong in reflection type, from invocation elements. Error: " + e.toString());
    ...
    public void messageEvent(Integer groupId, String secret, VkCallbackMessage message,IBotUtils utils) {
    	messageEvent(groupId, message,utils);
    }
	...
	public boolean parse(JsonObject json) {
        ...
        VkCustomCallbackMessage c_message = null;
        
        CallbackMessage message = null;
        
        if(type.equalsIgnoreCase(CALLBACK_EVENT_MESSAGE_EVENT)) {
        	c_message = gson.fromJson(json, typeOfClass);
        	  //System.out.print("Out1 : " + c_message.toString());
        }else {
        	message = gson.fromJson(json, typeOfClass);
        	
           // System.out.println("Out2 : " + message.toString());
        }
        VkCallbackMessage message2 = null;
        if(c_message != null) {
        	message2 = (VkCallbackMessage)c_message.getObject();
        }
        ...
        switch (type) {
            ...
            case CALLBACK_EVENT_MESSAGE_EVENT:
            	messageEvent(c_message.getGroup_id(), c_message.getSecret(),
            			(VkCallbackMessage)c_message.getObject(), 
            			(IBotUtils)new BotClient.BotUtilsImpl().get(new Message()
            					.setFromId(message2.getUser_id()).setPeerId(message2.getPeer_id())
            					.setConversationMessageId(message2.getConversation_message_id())));
            	break;
